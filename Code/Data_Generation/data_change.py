...
with open('../../data/RPD__Part_I_Crime_14_Days.csv') as fin:
    with open('../../data/mid_output_14.csv','wb') as csvfile:
        ...
        add_field = ["IsLarceny","IsBurglary","IsMotor Vehicle Theft","IsRobbery","IsAggravated Assault","IsMurder","Ismor","Isnoon","Isnight"]
        ...
        for  row in reader:
        	...
        	#check whether x,y cornate fields are empty or not
        	if (row["X"] and row["Y"]):
		        ...
		        if geoid in geoid_count:
		        	...
		        	subdic_crm={"IsLarceny":0,"IsBurglary":0,"IsMotor Vehicle Theft":0,"IsRobbery":0,"IsAggravated Assault":0,"IsMurder":0}
...
with open('../../data/mid_output_14.csv') as fin:
    with open('../../data/output_14.csv','wb') as csvfile:

        reader = csv.DictReader(fin)
        fieldnames= reader.fieldnames
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
        add_field = ["IsLarceny","IsBurglary","IsMotor Vehicle Theft","IsRobbery","IsAggravated Assault","IsMurder"]
